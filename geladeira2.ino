...
(
  "nome", //Nome da sua rede Wi-Fi
  "senha", //Senha da sua rede Wi-Fi
  "broker.emqx.io",  //MQTT Broker 
  "Default",   //Username
  "",   //Código do Token 
  "geladeiramedicao2",     //Nome do client
  1883              //Porta MQTT
);
...
void loop() {
  ...
  if(estadoPorta == HIGH){ //Se estiver em HIGH, então a porta está aberta
    ...
    client.publish("geladeira02/estado", "ABERTA"); //Publicamos no tópico MQTT desta geladeira a informação de que ela está aberta
    ...
    client.publish("geladeira02/tempgel",tempgel); //Publicamos no tópico da geladeira via MQTT sua temperatura
    ...
    client.publish("geladeira02/estado", "FECHADA"); //Publicamos no seu tópico o seu estado via MQTT   
    dtostrf(temperaturaFechado, 1, 2, tempgel); //Convertemos a temperatura da geladeira de float para string
    client.publish("geladeira02/tempgel",tempgel); //Publicamos via MQTT a temperatura da geladeira
    ...
    if(ctrl == 1){ //Se a variável de controle for 1
      ...
      client.publish("geladeira02/tempsom",charsomm); //Publicamos via MQTT a média da temperatura perdida
      ...
      client.publish("geladeira02/tempp",chartempp); //Publicamos a temperatura perdida via MQTT   
    ...
    temperaturaFechado = sensors.getTempCByIndex(0); //Pegamos a temperatura da geladeira
